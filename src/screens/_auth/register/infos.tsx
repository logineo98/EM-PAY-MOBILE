...
const Infos: FC<any> = () => {
    ...
    const initial: userModel = { phone: "", name: "", firstname: "", birthday: new Date(new Date().getTime()) }
    ...
    useEffect(() => { if (next) { AsyncStorage.setItem("inputs", JSON.stringify(store)); navigation.navigate("account"); setNext(false); setClick(false) } }, [next, store]);
    ...
    const handle_login = () => {
        ...
        if (!(inputs as any)?.phone.includes(indicatif)) inputs.phone = indicatif + inputs.phone
        setStore(inputs)
        setNext(true)
        setClick(true)
    }
                ...
                <View style={styles.content}>
                    ...
                    <View >
                        ...
                        <View style={styles.formulaire}>
                            ...
                            <View>
                                <View style={[[styles.inputfield, { borderRadius: width * 0.015, paddingHorizontal: width * 0.015 }]]}>
                                    <AntDesign name='edit' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                    <TextInput value={inputs?.name} onChangeText={text => handleChangeMobile("name", text, setInputs)} placeholder={"Nom"} placeholderTextColor={"gray"} style={[styles.input, { paddingHorizontal: width * 0.02 }]} />
                                </View>
                            </View>
                            ...
                            <View>
                                <View style={[[styles.inputfield, { borderRadius: width * 0.015, paddingHorizontal: width * 0.015 }]]}>
                                    <AntDesign name='edit' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                    <TextInput value={inputs?.firstname} onChangeText={text => handleChangeMobile("firstname", text, setInputs)} placeholder={"Prenom"} placeholderTextColor={"gray"} style={[styles.input, { paddingHorizontal: width * 0.02 }]} />
                                </View>
                            </View>

                            <View>
                                <TouchableWithoutFeedback onPress={toggleModal} style={[styles.input, { padding: 15, flex: 1, }]} >
                                    <View style={[[styles.inputfield, { borderRadius: width * 0.015, paddingHorizontal: width * 0.015 }], { padding: 13, justifyContent: "flex-start" }]}>
                                        <MaterialCommunityIcons name='cake-layered' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                        <Text style={[{ color: colors.line }]}>  {format((inputs as any).birthday, 'dd/MM/yyyy')}</Text>
                                    </View>
                    ...
                    <View style={styles.btnbox}>
                        <Animated.View style={animatedStyle}>
                            <TouchableOpacity onPress={handle_login} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
    ...
    formulaire: { gap: 10 },
    btnbox: { alignItems: "flex-end", justifyContent: "flex-end" },
