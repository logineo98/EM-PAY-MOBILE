...
import { checking, login, reset_password } from '../../../libs/services/user/user.action'
...
const ResetPassword: FC<any> = () => {
    ...
    return (
        <Wrapper>
            ...
            <WrapperContent scoll position={"center"}>
                <View style={styles.content}>
                        ...
                        <View style={styles.formulaire}>
                            <View>
                                <View style={[[styles.inputfield, { borderRadius: width * 0.015, paddingHorizontal: width * 0.015 }]]}>
                                    ...
                                    <TextInput value={inputs?.password} onChangeText={text => handleChangeMobile("password", text, setInputs)} placeholder={connexion_screen.password_field} placeholderTextColor={"gray"} style={[styles.input, { paddingHorizontal: width * 0.02 }]} />
                                    <AntDesign name='eye' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                </View>
                                <Text style={css.auth.connexion.errortext}>{(error as any)?.login_password_error}</Text>
                            </View>

                            <View>
                                <View style={[[styles.inputfield, { borderRadius: width * 0.015, paddingHorizontal: width * 0.015 }]]}>
                                    <MaterialIcons name='fingerprint' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                    <TextInput value={inputs?.confirm} onChangeText={text => handleChangeMobile("confirm", text, setInputs)} placeholder={connexion_screen.password_field} placeholderTextColor={"gray"} style={[styles.input, { paddingHorizontal: width * 0.02 }]} />
                                    <AntDesign name='eye' style={{ fontSize: width * 0.06, color: colors.auth_icon }} />
                                </View>
                                <Text style={css.auth.connexion.errortext}>{(error as any)?.login_password_error}</Text>
                            </View>
                    ...
                    <View style={styles.btnbox}>
                        <TouchableOpacity onPress={() => navigation.navigate("forgot_verify")} activeOpacity={0.8}>
                            <LinearGradient style={styles.main_btn} colors={[colors.white, colors.black]}>
                                <AntDesign name='caretleft' style={styles.icon} />
                            </LinearGradient>
                        </TouchableOpacity>
                        <Animated.View style={animatedStyle}>
                            <TouchableOpacity onPress={handle_reset} activeOpacity={0.8}>
            ...
            {click && user_loading && <SecondaryLoader text={"Veuillez patienter! Réinitialisation du mot de passe en cours.."} />}
        </Wrapper>
    )
}
